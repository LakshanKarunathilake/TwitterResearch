...
export class AnalysisToDB {
  ...
  saveTweetSentiment(data) {
    data.doc.set(data.description_sentiment, { merge: true }).then(() => {
  ...
  getReplies(data) {
    let retweet_api_call =
      this.url + "/get_replies/" + data.screen_name + "/" + data.id_str;
  ...
  saveReplySentiment(data) {
    ...
    data.collection
      ...
      .doc(id)
      .set(data.descripiton_sentiment, { merge: true })
      .then(() => {
  ...
  getKeyWords(data) {
    ...
    this.http
      .post(watson_api_keywords, { text: data.text })
      .subscribe(words => {
        this.saveKeywords({
          words: words,
          doc: data.doc
  ...
  saveKeywords(data) {
    data.doc.set({ KeyWords: data.words }, { merge: true }).then(() => {
